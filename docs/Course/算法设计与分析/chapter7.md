...
实际做题中，就沿着起点出发一个个拓展标号，然后回溯修改流量。

注意最后没有增广链后的标号集A和U-A就是最小割。

![](img/2025-01-12-16-19-33.png)

改进：
- 保证所求增广链有一定性质（比如最短）
- 一次能求得多个增广链

记S出边流量之和为C，则复杂度为$O(mC)$。
...
dinic的复杂度为$O(n^3)$，对于简单容量网络（流量都为1），复杂度为$O(\sqrt{n} m)$
...
![](img/2025-01-12-21-52-16.png)

本质就是从未饱和点出发，如果一直走交错路径能够找到非饱和点，那就找到了增广路径。
